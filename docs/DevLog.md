...
## 2026-09-12：用户视角审查问题修复（R01～R16）

- R01：控制核验改用生成后回读，保留 setter 应用证据；摘要核验增加 `basis=post_generation`，应用失败、回读失败及不匹配分别记录。
- R07：在唯一控制注册表中保留 `row/target_points`，增加固定停用原因；实际控制解析拒绝该项，历史目录和快照仍可读取。
- R08：IGG 文件路径与 PATH 查找结果在切换运行目录前绝对化。
- R09：源码/Git/注册表来源在 IGG 启动前采集，输入哈希及大小使用运行目录副本；完成阶段按有效执行证据、产物及实际质量评估记录。
- R10：campaign 每次执行使用新的 attempt 目录，恢复和串行重试不复用旧产物；新矩阵记录停用控制并跳过，分析器区分停用与拓扑阻塞，保留旧结果解析。
- 新增 CLI 停用入口、相对启动器、原输入修改/移动、生成后回读变化以及 campaign 失败恢复的定向回归；质量规则版本更新为 2，中文报告标明生成后核验依据。
- R02/R06：质量字段先识别后解析，全局及逐叶排非法统计返回 UNKNOWN；修复 CGNS 同块嵌套扫描和闭合后正文保留。定向质量测试 33 项、流式测试 10 项通过，原 8 MiB 复现文件仅保留 695 字节质量正文。
- 内核执行契约与 campaign 定向测试：37 项及 16 个 subtests 通过。文档同步 Schema v4、目标点数停用、快捷映射、数据库 v3 迁移与密码命令 PYTHONPATH，标准库测试入口改用 unittest。
- R03/R04/R05：周期回收其他 Worker 的失联运行，新回收 ID 进入后处理队列；后处理失败同步预览终态并让 manifest 明确返回失败；SQLite 备份路径正确进行 URI 编码。显式迁移 0003 仅修正旧的后处理失败/预览等待记录并追加恢复事件。
- R07 平台：控制目录及草稿预检增加 `can_clear`，停用项仍可清除历史继承值，创建和重试拒绝有效快照包含该项。旧队列真实 CLI 失败路径发现 Windows 中文输出编码问题，Python 子进程明确使用 UTF-8。后端累计 50 项定向用例通过。
- R11/R12/R15：详情按自身及后处理状态刷新，终态补取最终数据；经验草稿保留基准版本，冲突不覆盖；查询及写操作 401 均触发认证恢复，已打开工作台隐藏但不卸载，草稿保持且不重放写操作。
- R13/R14 与编辑体验：微小质量值使用科学计数法，null/空白/非有限值不参与比较；非法数值文本也计入 dirty，能够撤销和放弃；明确失败的 manifest 停止等待。前端分组定向测试 34 项和 9 项通过（3 项随新改动再次验证），修改文件 ESLint 通过。
- 统一验证：内核 93 项及 65 个 subtests 通过；前端 72 项、生产构建通过。后端全套 113 项通过、4 项跳过，1 个旧预览夹具因内部状态函数变更修正后单项通过，合计覆盖 114 项。
- 真实浏览器验收补修 R11：QUEUED 时缓存的控制目录未随父运行成功更新。控制目录查询键加入父运行状态，新增缓存仍 fresh 时 QUEUED→SUCCEEDED 自动解锁回归；控制编辑器 12 项通过，按新代码重新构建前端。
- 真实闭环：隔离平台上传 Rotor37，基线与 `optimization.steps=200` 分支均 SUCCEEDED/PASS，点数均 1,464,289，后处理 COMPLETED、预览 READY；分支生成后回读 200 且 VERIFIED。浏览器保存中文经验 v1、下载 CGNS（与登记哈希一致）、冻结并整页刷新，满意节点和经验内容保持。
- 本地上线：确认 `.env` 指向 `runs/platform-dev` 且无在途任务/后处理，备份数据库、282 项登记产物及实际几何/产物/预览/日志，登记缺失和不匹配均为 0；数据库 v2→v3，原有 8 个成功运行保持。同步构建并启动本地 API/Worker，`http://127.0.0.1:8000` 健康检查正常，保留原共享账号配置。
- 备份与上线证据：`runs/implementation_20260912/deployment-result.json`；真实浏览器证据：同目录 `e2e-result.json` 与 `.playwright-cli/`。隔离验收服务和浏览器在验收后关闭，未创建分支或 Git 提交。
- 新机安装入口验收：Python `-S` 禁用 editable `.pth`，通过文档中的 PYTHONPATH 和普通依赖成功执行密码 CLI 模块；未读取或修改实际密码。完整修复对应及上线说明见 `docs/FIXES_20260912.md`。
- 本轮在当前分支实现，不访问 `archive/`；各模块验证与最终验收记录随实现补充。
